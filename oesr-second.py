...
def verify_pgp_passphrase(pw_file,identity):
    out = subprocess.check_output(
        [
            "gpg",
"--pinentry-mode", "loopback", "--batch", "--passphrase-file", pw_file , "--export-secret-keys", "-a", f"{identity}@oesr.local"
        ]
    ).decode()
...
def init_oesr(output_dir, people, threshold, num):
    ...
    for person in people:
        ...
        pgp_pub_path = export_pgp(f"{output_dir}/public/{person}.pgp.pub", oesr_config['people'][person]['identity'])
...
@click.option("-t", "threshold", required=True, type=int, help="The SSSS share threshold.")
def generate(people, output_dir, threshold):
    num = len(people)
    if threshold >= num:
        error(f"threshold must be smaller than the number of people: t/n = {threshold}/{num}")
    if threshold <= 1:
...
def verify(output_dir):
    ...
    for person, data in oesr_config['people'].items():
        ...
        for other_person, other_data in oesr_config['people'].items():
            if person != other_person:
                shares.append(read(f"{output_dir}/{other_person}/shares/{person}.share"))
        ...
        if secret_restored == secret_original:
            print("OK! restored secret (using threshold * shares) matches original secret")
